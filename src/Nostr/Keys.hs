...
import           MyCrypto       (KeyPair, Bip340Sig, XOnlyPubKey, keyPairFromSecKey, secKey, parseXOnlyPubKey)
...
instance FromJSON UnknownXOnlyPubKey where
  parseJSON = withText "unknown XOnlyPubKey" $ \t -> do
    case textToByteStringType t Schnorr.parseXOnlyPubKey of
      Just xo ->
...
instance Ord XOnlyPubKey where
  compare (Schnorr.XOnlyPubKey a) (Schnorr.XOnlyPubKey b) =
    compare a b

instance FromJSON XOnlyPubKey where
  parseJSON = withText "XOnlyPubKey" $ \p -> do
    case (textToByteStringType p Schnorr.parseXOnlyPubKey) of
      Just e -> return e
...
instance FromJSON Bip340Sig where
  parseJSON = withText "Bip340Sig" $ \s -> do
    case (textToByteStringType s Schnorr.bip340sig) of
      Just s' -> return s'
      _       -> fail "invalid schnorr sig"

instance ToJSON Bip340Sig where
  toJSON s = String $ T.pack $ Schnorr.exportBip340Sig s
